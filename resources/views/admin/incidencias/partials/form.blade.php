    ...
    <div class="form-group">
        ...
        <select class="form-control @error('asignado_id') is-invalid @enderror" name="asignado_id" id="asignado_id">
            ...
            @foreach ($tecnologiaUsers as $idUsuario => $nombre)
                {{-- <option value="{{ $tecnologiaUser/* ->idusuario */ }}" {{ old('asignado_id', $incidencia->asignado_id ?? '') == $tecnologiaUser/* ->idusuario */ ? 'selected' : '' }}>{{ $tecnologiaUser->id }} - {{ $tecnologiaUser/* ->nombre */ }}</option> --}}
                <option value="{{ $idUsuario/* ->idusuario */ }}" {{ old('asignado_id', $incidencia->asignado_id ?? '') == $idUsuario/* ->idusuario */ ? 'selected' : '' }}>{{ $idUsuario }} - {{ $nombre/* ->nombre */ }}</option>
            @endforeach
        </select>
